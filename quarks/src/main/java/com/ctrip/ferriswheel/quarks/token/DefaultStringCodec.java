...
public class DefaultStringCodec implements StringDecoder {
    private enum State {
        Normal,
        EscapePending,
        Unicode1,
    ...
    @Override
    public boolean isQuoteEnd(char ch, char quoteStart) {
        return isQuoteStart(quoteStart) && ch == quoteStart;
    }
    ...
    public static int decode(String source, int start, Appendable appendable) throws IOException {
        DefaultStringCodec codec = new DefaultStringCodec();
        if (!codec.isQuoteStart(source.charAt(start))) {
            throw new IllegalArgumentException();
        }

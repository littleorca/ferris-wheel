...
public class ChartConsultantHelper {
    ...
    public static ChartData getSuggestedChartModel(Table table, int left, int top, int right, int bottom) {
        ...
        chartData.setTitle(new DynamicVariantImpl(Value.str("Chart-" + System.currentTimeMillis())));
        ...
        if (rc.getRight() - rc.getLeft() >= rc.getBottom() - rc.getTop()) {
            ...
            if (rc.getTop() > top) {
                int row = rc.getTop() - 1;
                chartData.setCategories(new DynamicVariantImpl(formula(table, rc.getLeft(), row, rc.getRight(), row)));
            }
            // scan series
            ...
            for (int row = rc.getTop(); row <= rc.getBottom(); row++) {
                ...
                if (rc.getLeft() - 1 >= left) {
                    s.setName(new DynamicVariantImpl(formula(table, row, rc.getLeft() - 1)));
                }
                s.setyValues(new DynamicVariantImpl(formula(table, rc.getLeft(), row, rc.getRight(), row)));
            ...
            if (rc.getLeft() > left) {
                ...
                chartData.setCategories(new DynamicVariantImpl(formula(table, col, rc.getTop(), col, rc.getBottom())));
            }
            // scan series
            chartData.setSeriesList(new ArrayList(rc.getBottom() - rc.getTop() + 1));
            for (int col = rc.getLeft(); col <= rc.getRight(); col++) {
                ChartData.SeriesImpl s = new ChartData.SeriesImpl();
                if (rc.getTop() - 1 >= top) {
                    s.setName(new DynamicVariantImpl(formula(table, rc.getTop() - 1, col)));
                }
                s.setyValues(new DynamicVariantImpl(formula(table, col, rc.getTop(), col, rc.getBottom())));
                chartData.getSeriesList().add(s);
            }
        }
